...
Module for using Google Gemini API to cross-validate gene/variant-disease associations.
...
Valid models: gemini-1.5-pro, gemini-1.5-flash, gemini-2.0-flash
...
def estimate_tokens(text: str) -> int:
    ...
    Uses: ~4 characters = 1 token as per Gemini guidelines (and usual English text estimates)
...
# -------------------------
async def resilient_gemini_call(
    api_key: str,
    query: str,
    ...
    """
    Single async API call to Gemini with retry logic.
    Uses asyncio.to_thread to wrap the synchronous google-generativeai call.
    ...
    try:
        ...
        logger.error("google-generativeai package not available. Install: pip install google-generativeai")
        return None
...
# Main Async Function
# -------------------------
async def gemini_API_search_async(
    query: str,
    candidates: Dict,
    api_key: str,
    verbose: bool = False,
    ...
    try:
        ...
        logger.error("google-generativeai package not available. Install: pip install google-generativeai")
        return {
            "error": "google-generativeai import failed",
            "confirmed": [],
            "uncertain": [],
            "rejected": [],
            "not_assessed": [],
    ...
    if enable_chunking and len(chunks) > 1:
        ...
        async def limited_call(chunk):
            async with semaphore:
                return await resilient_gemini_call(
                    api_key,
                    query,
                    chunk,
                    model=model,
        ...
        verdict = await resilient_gemini_call(
            api_key,
            query,
            chunks[0],
            model=model,
