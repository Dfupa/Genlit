...
Module for using Anthropic Claude API to cross-validate gene/variant-disease associations.
...
Valid models: claude-opus-4-5, claude-sonnet-4-5, claude-haiku-3-5
...
def estimate_tokens(text: str) -> int:
    ...
    Uses: ~4 characters = 1 token as per Claude guidelines (and usual English text estimates)
...
async def resilient_claude_call(
    ...
    """
    Single async API call to Claude with retry logic.
    ...
    for attempt in range(max_retries):
        try:
            response = await client.messages.create(
                ...
                system="You are a biomedical genetics expert. Always respond with valid JSON only, no markdown code blocks, no explanation — only a raw JSON object matching the required schema.",
...
# Main Async Function
# -------------------------
async def claude_API_search_async(
    query: str,
    candidates: Dict,
    api_key: str,
    verbose: bool = False,
    ...
    try:
        ...
        logger.error("anthropic package not available. Install: pip install anthropic")
        return {
        ...
        if enable_chunking and len(chunks) > 1:
            ...
            async def limited_call(chunk):
                async with semaphore:
                    return await resilient_claude_call(
                        client,
                        query,
                        chunk,
                        model=model,
            ...
            verdict = await resilient_claude_call(
                client,
                query,
                chunks[0],
                model=model,
